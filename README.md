...
| `RandomBotFriendsMinFriends` | `byte` (0-255) | `20` | Нижняя граница случайной цели по числу друзей для каждого бота. |
| `RandomBotFriendsMaxFriends` | `byte` (0-255) | `50` | Верхняя граница случайной цели по числу друзей для каждого бота. |
...
Если `MinFriends` больше `MaxFriends`, значения меняются местами автоматически. Заявки отправляются только между ботами, залогиненными в Steam в момент проверки, и только тем, с кем ещё нет никаких отношений (не друг, заявка не отправлена/не получена, не заблокирован).
